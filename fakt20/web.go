...
func (d *webData) addUsersWeb(w http.ResponseWriter, r *http.Request) {
	...
	if u.FirstName != "" {
		pid, _ := db.QueryForLastUID(d.PDB)
...
func (d *webData) modifyUsersWeb(w http.ResponseWriter, r *http.Request) {
	...
	if changed {
		db.UpdateUser(d.PDB, p[d.indexNR])
	}
}
...
func (d *webData) webBillSelectUser(w http.ResponseWriter, r *http.Request) {
	d.Users = db.QueryAllUserInfo(d.PDB)
	ip := r.RemoteAddr
	...
	highestBillNR, totalLineCount := db.QueryLastBillID(d.PDB)
	...
	if buttonAction == "add new bill" {
		...
		d.CurrentBillID = db.AddBill(d.PDB, newBill)
...
func (d *webData) webBillLines(w http.ResponseWriter, r *http.Request) {
	...
	BillsForUser := db.QueryBillsForUser(d.PDB, d.ActiveUserID)
	...
	billLines := db.QueryBillLines(d.PDB, d.CurrentBillID)
