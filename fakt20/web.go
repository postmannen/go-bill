...
func (d *webData) modifyUsersWeb(w http.ResponseWriter, r *http.Request) {
	...
	p := db.QueryDBForAllUserInfo(d.PDB)
	...
	if changed {
		db.UpdateUserInDB(d.PDB, p[d.indexNR])
	}
...
func (d *webData) webBillSelectUser(w http.ResponseWriter, r *http.Request) {
	d.Users = db.QueryDBForAllUserInfo(d.PDB)
	ip := r.RemoteAddr
	...
	if buttonAction == "add new bill" {
		...
		db.AddBillLineToDB(d.PDB, billLine)
	}
}
...
func (d *webData) webBillLines(w http.ResponseWriter, r *http.Request) {
	...
	BillsForUser := db.QueryDBForBillsForUser(d.PDB, d.ActiveUserID)
	...
	billLines := db.QueryDBForBillLinesInfo(d.PDB, d.CurrentBillID)
